function res = find_wireless_sess_T_J(sess, varargin)
...
for ss = 0:(ndays-1);
    this_datestr = datestr(datenum(sessiondate) - ss,'yymmdd'); % adjust to bdata data format
    
    for rr = 1:length(ratlist) % make sure there is a match
        ...
        for ff=1:length(ratfiles)
            ...
            warning('off')
            load(this_behav_path, 'saved_history'); %load the behavior info
            warning('on')
            parsed_events   = saved_history.ProtocolsSection_parsed_events; % gives you pokes/waves/states for each behav ttl
            % load behavior TTLS for alignment with ephys
            ttls_fsm = nan(1,length(parsed_events)); % make empty list length of behavior ttls
            ...
            rt{inc}     = polyfit(ttl_trodesall,ttls_fsmall,1); % linear fit between ttls of both systems
            
            ttl_trode_fsm = ttl_trodesall*rt{inc}(1)+rt{inc}(2); % y = mx + b | function to go grom spike times to behavior times
